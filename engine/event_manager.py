    ...
    async def _run_event_loop(self):
        try:
            while self.active_event and not self.is_stopped:
                # 1. Filter choices based on requirements
                filtered_choices = []
                for choice in self.current_scene.choices:
                    if choice.required_points is not None and self.points < choice.required_points:
                        continue
                    if choice.requires_flag is not None and choice.requires_flag not in self.flags:
                        continue
                ...
                # If it's an ending scene or no valid choices are available, stop here
                if self.current_scene.is_ending or not filtered_choices:
                    if not self.current_scene.is_ending and not filtered_choices:
                        # Dead end warning
                        await self.event_channel.send(embed=EmbedBuilder.error_embed("🏁 لم تتوفر خيارات صالحة للمجموعة بناءً على الشروط، انتهت المغامرة."))
                    else:
                ...
                # 3. Setup Voting
                view = VotingView(choices=filtered_choices, timeout=self.voting_timeout)
                message = await self.event_channel.send(embed=embed, view=view)
                view.message = message

                # 4. Wait for voting to finish
                ...
                # Update group session states from winning choice
                self.points += winning_choice.points_reward

                if winning_choice.sets_flag is not None:
                    self.flags.add(winning_choice.sets_flag)

                if winning_choice.reputation is not None:
                    if winning_choice.reputation not in self.reputation:
                        self.reputation[winning_choice.reputation] = 0
                    self.reputation[winning_choice.reputation] += 1
