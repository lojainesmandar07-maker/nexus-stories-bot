...
class StoryManager:
    ...
    def load_story(self, filepath: str):
        try:
            ...
            # Check if it's a modern story with nodes format (solo or multiplayer)
            elif "id" in data and "nodes" in data:
                 world_aliases = {
                     "الفانتازيا": "fantasy",
                     "الماضي": "past",
                     "المستقبل": "future",
                 ...
                 mapped_world = world_aliases.get(raw_world, raw_world) if raw_world else None

                 story_theme = data.get("theme", "عام")
                 story_world = data.get("world_type", mapped_world or ("multi" if data.get("game_mode") == "multi" else "solo"))
    ...
    def _parse_and_add_story(self, data: dict, theme: str, world_type: str):
        try:
            ...
            if story_id in self.stories:
                print(
                    f"WARNING: Hash collision for story id={raw_id} (hash={story_id}). Skipping."
                )
                return
            ...
            print(f"Loaded Nested Story {story.id}")

        except Exception as e:
            print(f"Error parsing nested story {data.get('id', 'unknown')}: {e}")
    ...
    def _parse_and_add_old_story(self, data: dict):
        ...
        self.stories[story.id] = story
        print(f"Loaded Old Story {story.id}")
    ...
    def resolve_story(self, story_ref: int | str, game_mode: str | None = None) -> Optional[Story]:
        ...
        # Extract numeric ID from "Title (ID)" format (autocomplete label format)
        import re
        if isinstance(story_ref, str):
            paren_match = re.search(r'\((\d+)\)\s*$', story_ref.strip())
            if paren_match:
                extracted_id = int(paren_match.group(1))
                story = self.stories.get(extracted_id)
