...
class ShareEndingView(discord.ui.View):
    ...
    async def share_btn(self, interaction: discord.Interaction):
        ...
        # Disable buttons immediately and edit the message
        ...
        if endings_ch_id:
            try:
                ...
                if channel:
                    ...
                    await interaction.followup.send("✅ تم مشاركة نهايتك بنجاح في القناة المخصصة!", ephemeral=True)
                else:
                    await interaction.followup.send("❌ القناة المخصصة للمشاركات غير موجودة.", ephemeral=True)
            except Exception as e:
                print(f"Error sharing: {e}")
                await interaction.followup.send("⚠️ لم نتمكن من مشاركة النهاية الآن.", ephemeral=True)
        else:
            await interaction.followup.send("❌ لم يتم إعداد قناة المشاركات في النظام.", ephemeral=True)
    ...
    async def no_share_btn(self, interaction: discord.Interaction):
        ...
        for child in self.children:
            child.disabled = True
        await interaction.response.edit_message(view=self)
        await interaction.followup.send("تفهمنا ذلك، استمتع برحلتك القادمة! 👋", ephemeral=True)
